    ...
    class DiaryViewModel : ViewModelBase
    {
        ...
        private void OnDeleteCommandExecuted(object p)
        {
            try
            {
                ...
                if (Model.IsDir == 0) Delete(new ObservableCollection<Diary>() { Model });
                else Delete(new RecursionByCollection(Collection.OfType<ITreeModel>().ToObservableCollection(), Model)
                            .GetItemChilds().OfType<Diary>().ToObservableCollection());
                db.SaveChanges();
            }
            catch (Exception e)
            {
        ...
        private void OnSaveCommandExecuted(object p)
        {
            try
            {
                ...
                if (SelectedGroup != null) Model.ParentId = ((Diary)SelectedGroup).Id;
        ...
        public ICollection<Diary> Group { get; set; }
        ...
        private DiaryWindow Window;
        private ObservableCollection<Diary> GetCollection() => db.Diaries.OrderBy(d => d.Name).ToObservableCollection();
        private void CreateNewWindow() => Window = new DiaryWindow();
        private Diary CreateNewModel() => new Diary();

        private Diary GetModelById(int id)
        {
        ...
        private void Delete(ObservableCollection<Diary> collection)
        {
