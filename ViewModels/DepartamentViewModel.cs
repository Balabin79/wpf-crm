...
{
    class DepartamentViewModel : ViewModelBase
    {
        ...
        private void OnDeleteCommandExecuted(object p)
        {
            try
            {
                ...
                if (Model.IsDir == 0) Delete(new ObservableCollection<Departament>() { Model });
                else Delete(new RecursionByCollection(Collection.OfType<ITreeModel>().ToObservableCollection(), Model)
                            .GetItemChilds().OfType<Departament>().ToObservableCollection());
                db.SaveChanges();
            }
            catch (Exception e)
            {
        ...
        private void OnSaveCommandExecuted(object p)
        {
            try
            {
                ...
                if (SelectedGroup != null) Model.ParentId = ((Departament)SelectedGroup).Id;
                ...
                switch (param)
                {
                    ...
                    default:
                        ...
                        Group = new RecursionByCollection(Collection.OfType<ITreeModel>().ToObservableCollection(), Model)
                            .GetDirectories().OfType<Departament>().ToObservableCollection();
        ...
        public ICollection<Departament> Group { get; set; }
        ...
        /******************************************************/
        public ObservableCollection<Departament> Collection
        {
            get => _Collection;
            set => Set(ref _Collection, value);
        }
        public Departament Model { get; set; }
        ...
        private ObservableCollection<Departament> _Collection;
        private DepartamentWindow Window;
        private ObservableCollection<Departament> GetCollection() => db.Departaments.OrderBy(d => d.Name).ToObservableCollection();
        private void CreateNewWindow() => Window = new DepartamentWindow();
        private Departament CreateNewModel() => new Departament();

        private Departament GetModelById(int id)
        {
        ...
        private void Delete(ObservableCollection<Departament> collection)
        {
