    ...
    public class ComplaintViewModel : ViewModelBase
    {
        ...
        public ComplaintViewModel()
        {
            try
            {
                ...
                Collection = db?.Complaints.Include(f => f.Parent).OrderBy(f => f.Name).ToObservableCollection() ?? new ObservableCollection<Complaint>();
            }
            catch
            {
                ThemedMessageBox.Show(title: "Ошибка", text: "Данные в базе данных повреждены! Программа может работать некорректно с разделом \"Шаблоны жалоб пациента\"!",
                        messageBoxButtons: MessageBoxButton.OK, icon: MessageBoxImage.Error);
            }
        }

        public ObservableCollection<Complaint> Collection
        {
        ...
        public void OpenForm(object p)
        {
            try
            {
                ...
                Model = (param > 0) ? db.Complaints.Where(f => f.Id == param).Include(f => f.Parent).FirstOrDefault() : new Complaint();
                Model.IsDir = (param < 0) ? param == -1 ? 0 : 1 : Model.IsDir;
                VM = new ComplaintMediatorVM(db, Model.Id)
                {
                    Name = Model.Name,
        ...
        public void SelectItemInField(object p)
        {
            try
            {
                ...
                {
                    if (parameters.Tree.FocusedRow is Complaint item)
                    {
        ...
        public void Delete(object p)
        {
            try
            {
                ...
                {
                    var collection = new RecursionByCollection(Collection.OfType<ITreeModel>().ToObservableCollection(), Model).GetItemChilds().OfType<Complaint>().ToObservableCollection();
        ...
        public ComplaintMediatorVM VM { get; set; }
        public Complaint WithoutCategory { get; set; } = new Complaint() { Id = 0, IsDir = 1, ParentId = 0, Name = "Без категории", Guid = "000" };
        public Complaint Model { get; set; }
