    ...
    public class TreatmentPlanViewModel : ViewModelBase
    {
        ...
        public TreatmentPlanViewModel()
        {
            try
            {
                ...
                Collection = db?.TreatmentPlans.Include(f => f.Parent).OrderBy(f => f.Name).ToObservableCollection() ?? new ObservableCollection<TreatmentPlan>();
            }
            catch
            {
                ThemedMessageBox.Show(title: "Ошибка", text: "Данные в базе данных повреждены! Программа может работать некорректно с разделом \"Шаблоны планов лечения\"!",
        ...
        public void OpenForm(object p)
        {
            try
            {
                ...
                Model = (param > 0) ? db.TreatmentPlans.Where(f => f.Id == param).Include(f => f.Parent).FirstOrDefault() : new TreatmentPlan();
                Model.IsDir = (param < 0) ? param == -1 ? 0 : 1 : Model.IsDir;
                VM = new TreatmentPlanMediatorVM(db, Model.Id)
                {
                    Name = Model.Name,
        ...
        public void SelectItemInField(object p)
        {
            try
            {
                ...
                {
                    if (parameters.Tree.FocusedRow is TreatmentPlan item)
                    {
        ...
        public void OpenByParentForm(object p)
        {
            try
            {
                ...
                var model = db.TreatmentPlans.Where(f => f.Id == param).Include(f => f.Parent).FirstOrDefault();
                if (model?.IsDir == 0)
                {

                    VM = new TreatmentPlanMediatorVM(db)
                    {
                        ParentId = model.ParentId,
                        Parent = model.Parent,
                        IsDir = 0,
        ...
        public void Delete(object p)
        {
            try
            {
                ...
                {
                    var collection = new RecursionByCollection(Collection.OfType<ITreeModel>().ToObservableCollection(), Model).GetItemChilds().OfType<TreatmentPlan>().ToObservableCollection();
        ...
        public TreatmentPlanMediatorVM VM { get; set; }
        public TreatmentPlan WithoutCategory { get; set; } = new TreatmentPlan() { Id = 0, IsDir = 1, ParentId = 0, Name = "Без категории", Guid = "000" };
