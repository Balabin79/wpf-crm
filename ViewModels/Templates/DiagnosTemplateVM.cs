...
{
    public class DiagnosTemplateVM : BindableBase, IDataErrorInfo
    {
        public DiagnosTemplateVM(ApplicationContext db, int id = 0) => Diagnoses = db?.Diagnoses.Where(f => f.IsDir == 1 && f.Id != id).Include(f => f.Parent)
        ...
        public ObservableCollection<Diagnos> Diagnoses
        {
