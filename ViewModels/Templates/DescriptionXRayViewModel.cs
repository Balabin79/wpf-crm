...
{
    public class DescriptionXRayViewModel : ViewModelBase
    {
        private readonly ApplicationContext db;
        public DescriptionXRayViewModel()
        {
            try
            {
                db = new ApplicationContext();
                Collection = db?.DescriptionXRay.Include(f => f.Parent).OrderBy(f => f.Name).ToObservableCollection() ?? new ObservableCollection<DescriptionXRay>();
            }
            catch
            {
                ThemedMessageBox.Show(title: "Ошибка", text: "Данные в базе данных повреждены! Программа может работать некорректно с разделом \"Шаблоны описания рентгеновских снимков\"!",
        ...
        public void OpenForm(object p)
        {
            try
            {
                ...
                Model = (param > 0) ? db.DescriptionXRay.Where(f => f.Id == param).Include(f => f.Parent).FirstOrDefault() : new DescriptionXRay();
                Model.IsDir = (param < 0) ? param == -1 ? 0 : 1 : Model.IsDir;
                VM = new DescriptionXRayMediatorVM(db, Model.Id)
                {
                    Name = Model.Name,
        ...
        public void SelectItemInField(object p)
        {
            try
            {
                ...
                {
                    if (parameters.Tree.FocusedRow is DescriptionXRay item)
                    {
        ...
        public void OpenByParentForm(object p)
        {
            try
            {
                ...
                var model = db.DescriptionXRay.Where(f => f.Id == param).Include(f => f.Parent).FirstOrDefault();
                if (model?.IsDir == 0)
                {

                    VM = new DescriptionXRayMediatorVM(db)
                    {
                        ParentId = model.ParentId,
                        Parent = model.Parent,
                        IsDir = 0,
