...
namespace Dental.ViewModels.Materials
{
    class MaterialViewModel : TreeBaseViewModel<Nomenclature>
    {
        public MaterialViewModel(ApplicationContext ctx, DbSet<Nomenclature> context) : base(ctx, context)
        {
            ...
            {
                ThemedMessageBox.Show(title: "Ошибка", text: "Данные в базе данных повреждены! Программа может работать некорректно с разделом \"Материалы\"!", messageBoxButtons: MessageBoxButton.OK, icon: MessageBoxImage.Error);
            }
        }
