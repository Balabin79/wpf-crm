...
{
    class AdvertisingViewModel : ViewModelBase, ICollectionCommand
    {
        ...
        public AdvertisingViewModel()
        {
            ...
            Repository.CopyModel += ((IModel, TableView) c) => {
                var copiedRow = Collection.Where(d => d.Id == ((Advertising)c.Item2.FocusedRow)?.Id).FirstOrDefault();
                if (copiedRow != null)
                {
                    int index = Collection.IndexOf(copiedRow) + 1;
                    Collection.Insert(index, (Advertising)c.Item1);
            ...
            Repository.UpdateModel += ((IModel, TableView) c) => {
                ...
                if (row != null)
                {
                    ...
                    Collection[index] = (Advertising)c.Item1;
                }
            };
            Repository.AddModel += ((IModel, TableView) c) => {
                Collection.Add((Advertising)c.Item1);
                var row = Collection.Where(d => d.Id == c.Item1.Id).FirstOrDefault();
        ...
        AdvertisingRepository Repository { get; set; }
