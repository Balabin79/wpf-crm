...
{
    class PriceViewModel : DevExpress.Mvvm.ViewModelBase
    {
        private readonly ApplicationContext db;
        public PriceViewModel(Employee emp)
        {
            try
            {
                ...
                if (Collection?.Count > 0) IndividualPriceVisible = Visibility.Visible; // тут коллекцию изменить на другую, это для примера
            }
            catch(Exception e)
            {
