...
{
    class DiscountGroupsViewModel : ViewModelBase, ICollectionCommand
    {
        ...
        public DiscountGroupsViewModel()
        {
            ...
            Repository = new DiscountGroupsRepository();
            Repository.CopyModel += ((IModel, TableView) c) => {
                var copiedRow = Collection.Where(d => d.Id == ((DiscountGroups)c.Item2.FocusedRow)?.Id).FirstOrDefault();
                if (copiedRow != null)
                {
                    int index = Collection.IndexOf(copiedRow) + 1;
                    Collection.Insert(index, (DiscountGroups)c.Item1);
            ...
            Repository.UpdateModel += ((IModel, TableView) c) => {
                ...
                if (row != null)
                {
                    ...
                    Collection[index] = (DiscountGroups)c.Item1;
                }
            };
            Repository.AddModel += ((IModel, TableView) c) => {
                Collection.Add((DiscountGroups)c.Item1);
                var row = Collection.Where(d => d.Id == c.Item1.Id).FirstOrDefault();
        ...
        DiscountGroupsRepository Repository { get; set; }
