    ...
    class SpecialityViewModel : ViewModelBase, ICollectionCommand
    {
        public SpecialityViewModel()
        {
            ...
            Repository.AddModel += ((IModel, TreeListView) c) =>
            {
                ...
                TreeListNode node;
                if (((Speciality)c.Item2.FocusedNode.Content).Dir == (int)TypeItem.Directory)
                {
                    node = c.Item2.FocusedNode.Nodes.Where(d => ((Speciality)d.Content).Id == c.Item1.Id).FirstOrDefault();
                }
                else
                {
                    node = c.Item2.FocusedNode.ParentNode.Nodes.Where(d => ((Speciality)d.Content).Id == c.Item1.Id).FirstOrDefault();
                }
                if (node != null)
                {
